...
window = tk.Tk()
window.geometry("980x700+300+100")
window.title("Typing Speed Test")
bg_color = "#4d345c"
window.config(bg=bg_color)

tk.Label(window, text="Typing Speed Test", anchor=tk.CENTER, font=(
    "times new roman", 50, "bold"), bg=bg_color, fg="#e9f241").pack(side=tk.TOP)

sentence_label = tk.Label(window, text="a set of words that is complete in itself, typically containing a subject and "
                                       "predicate, conveying a statement, question, exclamation, or command, "
                                       "and consisting of a main clause and sometimes one or more subordinate clauses.",
                          wraplength=700, anchor=tk.CENTER, font=("arial", 20, "bold"), bg=bg_color, fg="#ffffff",
...
typing_box = tk.Entry(window, font=("arial", 20, "bold"),
                      width=50, textvariable=text)
typing_box.place(x=110, y=450)

reset_button = tk.Button(window, text="Reset", font=(
    "arial", 18, "bold"), width=12, command=Reset, bg='#102859', fg='white')
reset_button.place(x=140, y=560)
change_button = tk.Button(window, text="Change Text", font=(
    "arial", 18, "bold"), width=12, command=get_sentence, bg='#102859', fg='white')
change_button.place(x=380, y=560)
result_button = tk.Button(window, text="Result", font=(
    "arial", 18, "bold"), width=12, command=result, bg='#102859', fg='white')
result_button.place(x=620, y=560)

Speed = tk.Label(window, text="Speed: 00 WPM", font=(
    "arial", 15, "bold"), bg=bg_color, fg="#ffffff")
Speed.place(x=155, y=630)
Accuracy = tk.Label(window, text="Accuracy: 00", font=(
    "arial", 15, "bold"), bg=bg_color, fg="#ffffff")
Accuracy.place(x=409, y=630)
Time = tk.Label(window, text="Time: 0 sec", font=(
    "arial", 15, "bold"), bg=bg_color, fg="#ffffff")
Time.place(x=660, y=630)
